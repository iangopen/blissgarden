import { PRESTIGE_PERKS, SEEDS } from './data.js';
import { STATE, prevReadyState, state, tileCount } from './state.js';
...
export function getPrestigePointsEarned() {
...
export function prestige() {
  ...
  // ── Apply headStart ───────────────────────────────────────────────────────
  const headStartPerk = (PRESTIGE_PERKS || []).find(p => p.id === 'headStart');
...
export function buyPerk(perkId) {
  ...
  recalculateModifiers();
  save();
  EventBus.emit('perk:purchased', { perkId });
}

// Ascension seeds cost prestige points (Stage 5). Returns true when bought.
export function buyAscensionSeed(key) {
