import { MILESTONE_VALS, SEEDS, STAGES, THORNED_WEED_CLICKS, VOID_RIFT_CLICKS, WEED_CLICKS } from '../data.js';
import { STATE, coinHTML, formatNumber, mk, state, ui } from '../state.js';
import { save } from '../save.js';
import { isReady } from '../upgrades.js';
import { EventBus } from '../core/bus.js';
import { checkAchievements } from '../achievements.js';
import { log, showBanner } from '../render/log.js';
import { addToSellQueue, canTick, tickSellBox } from './sellTick.js';
import { crowTick, hawkTick } from './crowHawk.js';
import { moleTick, moundTick } from './mole.js';
import { blightTick, locustTick, rootRotSpawnTick, rotTick, showRotCureMenu } from './blight.js';
import { fungalSpawnTick, fungalSpreadTick, showFungalCureMenu } from './fungal.js';
import { masterFarmerTick, thornedWeedTick, weedTick } from './weeds.js';
import { acidRainTick, claimedTileTick, diseasedTileTick, landDeveloperTick, plagueRatTick, showReclaimMenu } from './stage4.js';
import { cosmicCrowTick, realityStormTick, voidRiftEffectTick, voidRiftTick } from './stage5.js';
import { moveGhost, startDrag } from '../drag.js';
...
export function applyWater(idx) {
  ...
  EventBus.emit('tile:changed', idx); RenderPanel.renderInventory(); RenderPanel.renderItems();
  log(`💧 ${SEEDS[td.seed].name} watered (+25% value, +25% speed)`, 'growth');
  EventBus.emit('crop:watered');
  save();
}

export function drownTile(idx) {
  const td = state.tiles[idx];
  if (!td) return;
...
// ── LOOSE CROPS ──
// A crop lying on the ground at screen position (x, y), e.g. knocked loose by a mole.
export function dropLoose(seed, x, y, bonus = 1.0, drowned = false, fungal = false) {
  const id = ui.nextId++;
  state.loose.push({ seed, id, x, y, bonus, drowned, fungal });
  sfx.drop();
  EventBus.emit('loose:dropped', id);
  save();
}

// ── INVENTORY ──
export function addInventory(seed) { state.inventory[seed] = (state.inventory[seed] || 0) + 1; }

// ── BAG OPENING ──
export function openBag(bag) {
...
export function deselect() {
  if (ui.selectedTile !== null) { const p = ui.selectedTile; ui.selectedTile = null; EventBus.emit('tile:changed', p); }
}

export function hideTileMenu() { document.getElementById('tile-menu').style.display = 'none'; }

export function showTileMenu(idx, x, y) {
  ...
  if (isCaged) {
    ...
    btn.addEventListener('mousedown', e => {
      ...
      EventBus.emit('tile:changed', idx); RenderPanel.renderItems(); log('🔒 Cage removed, returned to inventory', 'system'); save(); hideTileMenu();
    });
  ...
  if (isFert) {
    ...
    btn.addEventListener('mousedown', e => {
      ...
      EventBus.emit('tile:changed', idx); save(); hideTileMenu();
    });
    menu.appendChild(btn);
  }
  if (isHiredHand) {
    ...
    btn.addEventListener('mousedown', e => {
      ...
      EventBus.emit('tile:changed', idx); RenderPanel.renderInventory(); save(); hideTileMenu();
    });
    menu.appendChild(btn);
  }
...
export function onTileDown(e) {
  ...
  if (state.voidRifts && state.voidRifts[idx] !== undefined) {
    ...
    if (state.voidRifts[idx].clicks >= VOID_RIFT_CLICKS) {
      ...
      EventBus.emit('tile:changed', idx);
    } else {
      e.currentTarget.classList.add('tile-rift-hit');
      setTimeout(() => EventBus.emit('tile:changed', idx), 80);
    }
    save(); return;
  }
  ...
  if (state.mounds && state.mounds[idx] !== undefined) {
    ...
    EventBus.emit('tile:changed', idx); save(); return;
  }
  ...
  if (isReady(td, idx)) {
    ...
    EventBus.emit('harvest:pop', { x: tileRect.left + tileRect.width / 2, y: tileRect.top + tileRect.height / 2, seed });
  ...
  e.stopPropagation();
  if (ui.selectedTile === idx) deselect();
  else { deselect(); ui.selectedTile = idx; EventBus.emit('tile:changed', idx); }
  ...
  if (state.rotTiles && state.rotTiles[idx] && state.rotTiles[idx].infectedAt !== undefined && state.rotTiles[idx].deadAt === undefined) {
    ...
    if (state.upgrades.fastCure) {
      if (state.coins >= cureCost) {
        ...
        updateCoins(); EventBus.emit('tile:changed', idx); save();
      } else {
