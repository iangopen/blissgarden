...
import { coinHTML, formatNumber, mk, state, tileCount } from '../state.js';
import { save } from '../save.js';
import { eventChance, isReady } from '../upgrades.js';
import { EventBus } from '../core/bus.js';
import { log, showBanner } from '../render/log.js';
import { dropLoose, getCurrentStage, hideTileMenu, updateCoins } from './index.js';
...
// ── RECLAIM MENU ──────────────────────────────────────────────────────────
export function showReclaimMenu(idx, cost, x, y) {
...
export function claimedTileTick() {
  ...
  Object.keys(state.claimedTiles).forEach(k => {
    ...
    if (cl.releasesAt !== undefined) {
      if (now >= cl.releasesAt) { delete state.claimedTiles[idx]; EventBus.emit('tile:changed', idx); changed = true; }
...
// ── PLAGUE RATS ───────────────────────────────────────────────────────────
export function plagueRatTick() {
  if (!state.mature || getCurrentStage().stage < 4) return;
  if (Math.random() < eventChance('plagueRat')) plagueRatAttack();
}

export function plagueRatAttack() {
  const cands = [];
  for (let i = 0; i < tileCount(); i++) {
...
export function diseasedTileTick() {
...
export function animateAcidRain() {
