import { BALANCE, SEEDS } from './data.js';
import { STATE, formatNumber, state } from './state.js';
import { save } from './save.js';
import { eventChance } from './upgrades.js';
import { TimerManager } from './core/bus.js';
import { checkMaturity, checkStages, getCurrentStage, updateCoins } from './events/index.js';
import { RenderHUD } from './render/hud.js';
import { getDayNightMult } from './daynight.js';
...
export const DebugPanel = (() => {
  ...
  function _buildDebugPanel() {
    ...
    bar.addEventListener('mousedown', e => {
      ...
      dragging = true; dragDx = e.clientX - panel.offsetLeft; dragDy = e.clientY - panel.offsetTop;
      e.preventDefault();
    });
  ...
  function _updatePriceTable() {
    ...
    const entries = Object.entries(SEEDS || {}).sort((a, b) => {
  ...
  function _updateDebugPanel() {
    ...
    const h   = (label, color) => `<div style="color:${color || '#6af'};font-weight:700;margin:6px 0 2px">${label}</div>`;
    const row = (k, v) => `<div><span style="color:#778">${k}:</span> ${v}</div>`;
    let html = '';
    html += h('TIMING');
  ...
  function _toggleDebugPanel() {
    if (_debugPanelEl && _debugPanelEl.style.display !== 'none') _closeDebugPanel();
    else _openDebugPanel();
  }

  // window.__bliss (all game exports) exists only while debug mode is on.
  let _consoleApi = null;
  function setConsoleApi(api) { _consoleApi = api; }
  ...
  function applyDebugMode() {
    ...
    if (on && _consoleApi) window.__bliss = _consoleApi;
    else delete window.__bliss;
  }
  ...
  return { applyDebugMode, setConsoleApi, setupUI };
})();
