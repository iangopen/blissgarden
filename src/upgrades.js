import { BALANCE, ITEMS, PRESTIGE_PERKS, SEEDS, UPGRADES } from './data.js';
import { STATE, coinHTML, state, tileCount } from './state.js';
import { save } from './save.js';
import { checkFreeRecipes } from './crafting.js';
import { applyArtifacts } from './artifacts.js';
...
export function getEffectiveSpeedMult(seedId, idx) {
  const gs       = STATE.modifiers.growSpeed    || 1;
  const season   = STATE.modifiers.seasonGrowMult || 1;
  const dayNight = getDayNightMult(seedId);
  let tile = 1.0;
  if (idx !== undefined) {
...
export function isReady(td, idx) {
  return (td.burnedSeconds ?? 0) >= SEEDS[td.seed].grow;
}

export function canCapacity()       { return STATE.upgrades.copperSpout ? 2 : 1; }
export function canFillTime()       { return STATE.upgrades.copperSpout ? 8000 : 20000; }
...
export function stageValue(v, stage) {
...
export function eventChance(id) {
...
export function eventInterval(id) { return stageValue(BALANCE.events[id].interval, getCurrentStage().stage); }
