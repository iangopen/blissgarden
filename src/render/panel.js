import { ACHIEVEMENTS, PRESTIGE_PERKS, SEEDS } from '../data.js';
import { STATE, fmt, formatNumber, mk, state } from '../state.js';
import { getCurrentStage } from '../events/index.js';
import { RenderSeeds } from './seeds.js';
import { RenderInventory } from './inventory.js';
import { RenderItems } from './items.js';
import { RenderUpgrades } from './upgrades.js';
import { buyAscensionSeed, buyPerk, canPrestige, getPrestigePointsEarned, prestige } from '../prestige.js';
...
export const RenderPanel = (() => {
  ...
  function renderAchievements() {
    ...
    const achs = ACHIEVEMENTS || [], unlocked = state.achievements || {};
    let totalUnlocked = 0;
  ...
  function renderPrestige() {
    ...
    _perkCards.forEach(({ stackEl, btn, effectEl, card }, perkId) => {
      const perk = (PRESTIGE_PERKS || []).find(p => p.id === perkId); if (!perk) return;
  ...
  // Subscriptions that replaced direct calls from logic (breaks prestige ↔ render/panel; crafting → panel).
  function start() {
    EventBus.on('inventory:changed', () => RenderInventory.renderInventory());
    EventBus.on('perk:purchased',    () => renderPrestige());
  }
