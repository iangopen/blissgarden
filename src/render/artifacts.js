import { ACHIEVEMENTS, ARTIFACTS, BLUEPRINTS, RECIPES } from '../data.js';
import { STATE, coinHTML, formatNumber, mk, state } from '../state.js';
import { craftArtifact, purchaseBlueprint } from '../artifacts.js';
...
export const RenderArtifacts = (() => {
  ...
  function _renderArtifactsTab(container) {
    ...
    (ARTIFACTS || []).forEach(art => {
      ...
      const bp       = (BLUEPRINTS || []).find(b => b.artifactId === art.id);
      ...
      if (!bpUnlocked && !crafted) {
        ...
        const hint = bp && bp.source === 'achievement'
          ? `Unlock via achievement: ${(ACHIEVEMENTS || []).find(a => a.id === bp.achievementId)?.name ?? bp.achievementId}`
        ...
        Object.entries(art.ingredients).forEach(([k, needed]) => {
          ...
          const recipe = (RECIPES || []).find(r => r.id === k);
          const tag = mk('span');
          tag.style.cssText =
    ...
    const activeArts = (ARTIFACTS || []).filter(a => STATE.artifacts[a.id]);
  ...
  function _renderBlueprintsTab(container) {
    ...
    const shopBps  = (BLUEPRINTS || []).filter(b => b.source === 'shop');
    const achBps   = (BLUEPRINTS || []).filter(b => b.source === 'achievement');
    ...
    if (achBps.length > 0) {
      ...
      achBps.forEach(bp => {
        const owned = !!STATE.blueprints[bp.id];
        const art   = (ARTIFACTS || []).find(a => a.id === bp.artifactId);
        const ach   = (ACHIEVEMENTS || []).find(a => a.id === bp.achievementId);
