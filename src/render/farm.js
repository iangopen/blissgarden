import { THORNED_WEED_CLICKS, VOID_RIFT_CLICKS, WEED_CLICKS } from '../data.js';
import { SPRITE_URL, fmt, getGridDims, makeSpriteDiv, mk, prevReadyState, state, tileCount, ui } from '../state.js';
import { save } from '../save.js';
import { isReady, remSec } from '../upgrades.js';
import { log } from './log.js';
import { deselect, onTileDown } from '../events/index.js';
import { DragSystem, moveGhost, startDrag } from '../drag.js';
...
// VISUAL HELPERS
// ══════════════════════════════
export function spawnHarvestPop(x, y, seed) {
...
export const RenderFarm = (() => {
      ...
      if (ui.selectedTile === idx) {
        ...
        if (isCaged) {
          ...
          cb.addEventListener('mousedown', e => {
            ...
            renderTile(idx); RenderPanel.renderItems(); log('🔒 Cage removed, returned to inventory', 'system'); save();
          });
          el.appendChild(cb);
        }
      }
  ...
  function probeSprites() {
    const img = new Image();
    img.onerror = () => console.error(`sprites.png not found at ${SPRITE_URL}`);
    img.src = SPRITE_URL;
  }

  // Logic emits these instead of importing render (breaks the events/* ↔ render/farm and drag ↔ render/farm cycles).
  function start() {
    EventBus.on('tile:changed',  idx => renderTile(idx));
    EventBus.on('grid:changed',  () => renderGrid());
    EventBus.on('loose:changed', () => renderLoose());
    EventBus.on('loose:dropped', id => {
      renderLoose();
      const el = document.querySelector(`.loose-crop[data-id="${id}"]`);
      if (el) el.classList.add('loose-land');
    });
    EventBus.on('harvest:pop', ({ x, y, seed }) => spawnHarvestPop(x, y, seed));
  }

  return { renderTile, renderGrid, buildGrid, probeSprites, tileNodes, start };
})();
